...
import { faFeather, faRightFromBracket } from '@fortawesome/free-solid-svg-icons';
  ...
  return (
    <div className=' fixed w-full flex items-center justify-center h-[80px] shadow-md shadow-green-600 z-[1000] bg-background'>
        <div className='flex items-end gap-3'>
            ...
            <FontAwesomeIconWrapper icon={faFeather} style={{height:'40px'}} color='green' />
        </div>
        <div className='float-right flex gap-2 px-3 absolute right-0'>
              <span title='Logout'>
                <FontAwesomeIconWrapper className="p-2 border rounded bg-gray-200 dark:bg-gray-800 cursor-pointer" icon={faRightFromBracket} onClick={handleLogout} />
              </span>
              {process.env.NODE_ENV === 'development' && <div title='Theme change'>
                <ThemeToggle />
              </div>}
        </div>
    </div>
