  ...
  const onSubmit = async (data: loginSchemaType) => {
    try {
      const res = await fetch(
        `/api/authentication/${isLogin ? "login" : "signup"}`,
        {
          method: "POST",
        ...
        <div className="flex flex-col gap-8 p-5 border-none rounded-md shadow-sm shadow-gray-400 w-1/4">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col">
              ...
              <input
                className="border border-green-500 bg-background rounded-md pl-4 pr-4"
                type="email"
                {...register("email")}
              />
          ...
          <div className="text-center">
            <button
              className="w-full bg-blue-600 rounded-md text-white"
              type="submit"
            >
              {submitBtnText}
            </button>
